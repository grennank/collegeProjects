...
e2004 <- as.data.frame(read.px("Datasets/E2004.px"))
# make it wide format (requires reshape2 package)
e2004_shorter <- subset(e2004, select = -c(Aggregate.Town.or.Rural.Area, Sex))
e2004$Stat <- paste(e2004$Statistic, "-", e2004$CensusYear)
e2004_shorter <- subset(e2004, select = -c(Aggregate.Town.or.Rural.Area, Sex, CensusYear, Statistic))

e2004_wide <- dcast(e2004_shorter, County.and.City~Stat, value.var = 'value',sum)
# then remove non-admin counties aggregates: counties, state, provinces#
# requires (dplyr package)
e2004_wide_admin <- as.data.frame( filter(e2004_wide, !(County.and.City %in%
...
e2004_wide_admin$County.and.City <- factor(factor_rename_counties_long(e2004_wide_admin$County.and.City))
e2004_map$COUNTYNAME <- factor(factor_rename_counties_long(e2004_map$COUNTYNAME))

# strange errors on merge with factors, so forcing to string
counties$COUNTYNAME <- as.vector(counties$COUNTYNAME)
e2004_wide_admin$County.and.City <- as.vector(e2004_wide_admin$County.and.City)
...
e2004_wide_admin$County.and.City[e2004_wide_admin$County.and.City == "Dún Laoghaire-Rathdown"] <- "Dún Laoghaire-Rathdown"

# check available data column names
colnames(e2004_wide_admin)

# merge map and data
e2004_map <- left_join(counties, e2004_wide_admin, by=c("COUNTYNAME" = "County.and.City"))
e2004_map$COUNTYNAME <- factor(e2004_map$COUNTYNAME)

# define a base map layer
housingplot_baselayer <- ggplot(e2004_map) + 
  aes(long, lat, group=group) +
  geom_polygon(colour="grey")

# plot a data column (vacancy rates 2016 %)
housingplot_baselayer + aes(fill=`Population (Number) - 2016`)
